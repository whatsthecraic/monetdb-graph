...
AC_DEFUN([_MONETDB_VALIDATE_OPTIONS], [dnl
  m4_pushdef([_MONETDB_OPTION_CURRENT], [[$2]])dnl
if m4_map_sep([_MONETDB_SHELL_TEST_NE], [ && ],  m4_split([$3])); then
  AC_MSG_ERROR([invalid value for the parameter $1. Possible choices are: m4_map_sep([m4_echo], [, ], m4_split([$3])). Given value: '$2'])
fi
  m4_popdef([_MONETDB_OPTION_CURRENT])dnl
])

# MONETDB_VALIDATE_OPTIONS(option_name, variable, list_options)
# -------------------------------------------
#
# Check whether <variable> contains one of the options of the value in
...
AC_DEFUN([MONETDB_VALIDATE_OPTIONS], [dnl
  m4_divert_push([INIT_PREPARE])dnl
  _MONETDB_VALIDATE_OPTIONS([$1], [$2], [$3])dnl
  m4_divert_pop([INIT_PREPARE])dnl
])

# _MONETDB_PRINT_CHOICE(arg)
# -------------------------------------------
#
# Echo the given argument
AC_DEFUN([_MONETDB_PRINT_CHOICE], [$1])

# MONETDB_ARG_ENABLE(option_name, help_description, list_options, default)
# -------------------------------------------
#
...
AC_DEFUN([MONETDB_ARG_ENABLE], [dnl
  ...
  m4_pushdef([_MONETDB_POSSIBLE_CHOICES], [m4_map_sep([_MONETDB_PRINT_CHOICE], [, ], m4_split([$3]))])
  AC_ARG_ENABLE([$1], AS_HELP_STRING([--m4_if([$4], [yes], [disable], [enable])-$1], 
    [$2. Possible choices are: _MONETDB_POSSIBLE_CHOICES. @<:@Default: $4@:>@]))    
  m4_popdef([_MONETDB_POSSIBLE_CHOICES])
