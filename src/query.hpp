...
class Query{
	// input parameters
	const vertex_t* query_perm; // projection ids for the source & destination tuples
...
public:
	Query(vertex_t* query_perm, vertex_t* query_src, vertex_t* query_dst, std::size_t query_sz,
		  vertex_t* output_filter, cost_t* output_cost, std::size_t output_capacity) :
			query_perm(query_perm), query_src(query_src), query_dst(query_dst), query_size(query_sz),
	...
	void join(std::size_t i) noexcept {
		assert(i < query_size && "Index out of bounds");
		output_last++;
		assert(output_last < output_capacity);
		output_filter[output_last] = query_perm ? query_perm[i] : i;
	}
